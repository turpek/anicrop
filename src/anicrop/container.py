...
from anicrop.geometry import GeometryStrategy, GroupGeometry, GeometryController, FitGroupGeometry
...
def _compute_layer_local_roi(target: LayerProtocol) -> Region | None:
...
def content_region(target: LayerProtocol) -> Region | None:
...
def global_content_region(
    container: LayerProtocol | Container | Sequence[LayerProtocol],
) -> Region | None:
    """Calcula a Bounding Box de conteúdo de todos os elementos projetada no Espaço Global."""
    def _extract(item: LayerProtocol | Container | Sequence[LayerProtocol]) -> Iterator[Region]:
        if isinstance(item, LayerProtocol):
